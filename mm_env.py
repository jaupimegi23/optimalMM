...
class SimpleEnv(gym.Env):
    ...
    def __init__(self,
                 T=10, dt=1, Q=3, dq=1, Q_0=0, dp=0.1, min_dp=1, s0=100, mu = 0, std=0.01, lambda_pos=1,
    ...
    def update_price(self):
        ...
        #print(np.random.normal(0, self.std))
        #self.mid += self.round_to_tick(np.random.normal(0, self.std))
        self.mid = self.mid*np.exp((self.mu-0.5*self.std**2)*self.dt + self.std*np.random.normal(0,np.sqrt(self.dt)))
